...
<section class="content">
  <div class="container-fluid">
    <div class="block-header">
      <h2>SUB CATEGORY MANAGEMENT(LEVEL 2)</h2>
    </div>
...
@include('categories.sub-categories2.modal')
...
<script type="text/javascript">
  var action_type = '';
  var ajaxURL = base_url+"/sub-category2"
  var SSPEnable = true
  var opt = {
    processing: true,
    serverSide: true,
    ajax:{
      url:'{{ url('sub-category2-all') }}',
      type:'POST',
    ...
    {data: 'sub_parent' , name: 'sub_parent'},
...
<script type="text/javascript">
  ...
  function setModal(data) {
    ...
    $('.form-line').addClass('focused');
        ...
        console.log(data);
        $('select[name="parent"]').selectpicker('val', data.parent.parent_id);
        getSubCategoryByMainCategory(data.parent_id);

        // $('select[name="sub_parent"]').selectpicker('val', data.parent_id);
        $('#formModal').modal('show');
    ...
    function clearModal() {
      update = 0
        ...
        $('select[name="sub_parent"]').selectpicker('val', 0);
    ...
    <script type="text/javascript">
      $('.js-basic-example').on('click', 'input[type="checkbox"]', function(event) {
        ...
        $.ajax({
          url: base_url+'/sub-category2-active',
          type: 'POST',
          dataType: 'JSON',
          data: {_token: token,sub_category2_id: id,active:active}
        })
        .done(function(data) {
    ...
    <script type="text/javascript">
      function getSubCategoryByMainCategory(selected_id = null){
        var parent = $("#parent").val();
        $.ajax({
          url: '{{url('sub-category2-getSubCategoryByMainCategory')}}',
          method: "post",
          data: {
            parent:parent,
          },
        ...
        success: function (data) {
          ...
          // $('#sub_parent').selectpicker('val', data.parent_id);
          if(selected_id){
            $('#sub_parent').selectpicker('val', selected_id);  
          }

          $('#sub_parent').selectpicker('refresh');
        },
        error: function(data){
        }
      });
