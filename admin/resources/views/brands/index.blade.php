...
<section class="content">
  <div class="container-fluid">
    ...
    <div class="row clearfix">
      ...
      <div class="col-lg-3 col-md-3 col-sm-6 col-xs-12">
        <div class="info-box bg-cyan hover-expand-effect">
          ...
          <div class="content">
            <div class="text">ACTIVE BRANDS</div>
            <span class="view-count">{{$activeBrand}}</span>
          </div>
        </div>
      </div>
      ...
      <div class="col-lg-3 col-md-3 col-sm-6 col-xs-12">
        <div class="info-box bg-cyan hover-expand-effect">
          ...
          <div class="content">
            <div class="text">INACTIVE BRANDS</div>
            <span class="view-count">{{$inActiveBrand}}</span>
          </div>
        </div>
      </div>
        ...
        <div class="card">
            <div class="header">
                <span class="pull-right">
                    ...
                    <a href="{{route('brands.create')}}" class="btn bg-light-blue waves-effect" data-toggle="tooltip" data-placement="left" title="" data-original-title="ADD NEW BRAND">
                ...
                <h2>
                  ALL Brands
                  <small>All the brand in the system</small>
                </h2>
              </div>
                      ...
                      <div class="row filter-box">
                        ...
                        <div class="col-sm-4">
                          <select class="form-control show-tick filter" id="statusFilter">
                              <option value="alltypes">-- All Brands --</option>
                              <option value="active">Active Brands</option>
                              <option value="inactive">In-active Brands</option>
                          </select>
                        </div>
                      </div>
                    </div>
   ...
   <div class="modal fade" id="viewBrandModal" tabindex="-1" role="dialog" aria-labelledby="">
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
          <div class="modal-header">
            <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
            <h4 class="modal-title" id="#">BRAND DETAILS</h4>
          </div>
  ...
  <script type="text/javascript">
    ...
    $('.js-basic-example').on('click', 'input[type="checkbox"]', function(event) {
      ...
      $.ajax({
        url: base_url+'/brand-active',
        type: 'POST',
        dataType: 'JSON',
        data: {_token: token,brand_id: id,status:status}
      })
      .done(function(data) {
  ...
  $('#btnClearFilter').click(function(event){    $('#startDateFilter').val("")
    $('#endDateFilter').val("")
    $('#moqFilter').selectpicker('val', "allbrands")
...
<script type="text/javascript">
  ...
  function viewBrandModal(id) {
    $.ajax({
     url: base_url + '/view-brand',
     type: 'POST',
     headers: {'X-CSRF-TOKEN': token},
     data: {id: id},
     beforeSend: function () {
      $("#viewBrandModal").find(".preloader-parent").fadeIn();
      $("#brandDetails").hide();
    },
    complete: function () {
      $("#viewBrandModal").find(".preloader-parent").hide();
      $("#brandDetails").fadeIn();
    },
    success:function(data){
       // console.log(data);
       $('#brandDetails').html(data);
     }
   })
